...
/**
 * Sorts {@link ODMComparedItem ODMComparedItems} by similar item count.
 *
 */
public class ItemSimilarComparator implements Comparator<ODMComparedItem> {
    ...
    public int compare(ODMComparedItem item, ODMComparedItem compareItem) {
        if (item.getItem().compareODMItem(compareItem.getItem()).equals(ODMCompareType.IDENTICAL)) {
            ...
            for (List<ODMItem> items : item.getSimilarFormItems().values()) {
                itemCount += items.size();
            }
            for (List<ODMItem> items : compareItem.getSimilarFormItems().values()) {
