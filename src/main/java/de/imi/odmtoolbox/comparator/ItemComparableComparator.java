...
/**
 * Sorts {@link ODMComparedItem ODMComparedItems} by comparable item count.
 * 
 */
public class ItemComparableComparator implements Comparator<ODMComparedItem> {
    ...
    public int compare(ODMComparedItem item, ODMComparedItem compareItem) {
        if (item.getItem().compareODMItem(compareItem.getItem()).equals(ODMCompareType.IDENTICAL)) {
            ...
            for (List<ODMItem> items : item.getComparableFormItems().values()){
                    itemCount += items.size();
            }
            for (List<ODMItem> items : compareItem.getComparableFormItems().values()){
