...
/**
 * Sorts {@link ODMComparedItem ODMComparedItems} by matching item count.
 *
 */
public class ItemMatchingComparator implements Comparator<ODMComparedItem> {
    ...
    public int compare(ODMComparedItem item, ODMComparedItem compareItem) {
        if (item.getItem().compareODMItem(compareItem.getItem()).equals(ODMCompareType.IDENTICAL)) {
            ...
            for (List<ODMItem> items : item.getMatchingFormItems().values()) {
                itemCount += items.size();
            }
            for (List<ODMItem> items : compareItem.getMatchingFormItems().values()) {
