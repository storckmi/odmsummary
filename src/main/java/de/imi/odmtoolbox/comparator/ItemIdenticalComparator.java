...
/**
 * Sorts {@link ODMComparedItem ODMComparedItems} by identical item count.
 *
 */
public class ItemIdenticalComparator implements Comparator<ODMComparedItem> {
    ...
    public int compare(ODMComparedItem item, ODMComparedItem compareItem) {
        if (item.getItem().compareODMItem(compareItem.getItem()).equals(ODMCompareType.IDENTICAL)) {
            ...
            for (List<ODMItem> items : item.getIdenticalFormItems().values()) {
                itemCount += items.size();
            }
            for (List<ODMItem> items : compareItem.getIdenticalFormItems().values()) {
