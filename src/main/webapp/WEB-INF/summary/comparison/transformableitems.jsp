...
<c:set var="formWidth" value="${tableStructure.getTransformableFormWidth()}"/>
<c:if test="${studyForms != null && studyForms.isEmpty() == false && formWidth != null && formWidth.isEmpty() == false}"> 
    <div id="studyForms" style="width: 100%; float: left; margin-top: 10px;">
        ...
        <button style="margin-bottom: 10px;" onclick="tableToExcel('exportTransformableTable', 'TransformableItems', 'TransformableItems.xls')">Export to Excel</button>
        (only works in Firefox 28+ and Google Chrome 35+)       
        <table id="exportTransformableTable" border="1" rules="all" style="width: 95%; float: left;">
            ...
            <c:set var="transformable" value="<%=de.imi.odmtoolbox.model.ODMCompareType.TRANSFORMABLE%>"/>

            <c:forEach items="${comparedItems[transformable]}" var="comparedItem"> 
                <tr>
                    <td>
                    ...
                    <c:forEach items="${comparedItem.getTransformableFormItems()}" var="form">
...
<c:if test="${studyForms == null || studyForms.isEmpty() == true || formWidth == null || formWidth.isEmpty() == true}">
    <p>The submitted ODM-Forms do not contain any transformable items.</p>
</c:if>  