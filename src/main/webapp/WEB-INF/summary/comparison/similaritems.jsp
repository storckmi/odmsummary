...
<c:set var="formWidth" value="${tableStructure.getSimilarFormWidth()}"/>
<c:if test="${studyForms != null && studyForms.isEmpty() == false && formWidth != null && formWidth.isEmpty() == false}">
    <div id="studyForms" style="width: 100%; float: left; margin-top: 10px;">
        ...
        <button style="margin-bottom: 10px;" onclick="tableToExcel('exportSimilarTable', 'SimilarItems', 'SimilarItems.xls')">Export to Excel</button>
        (only works in Firefox 28+ and Google Chrome 35+)
        <table id="exportSimilarTable" border="1" rules="all" style="width: 95%; float: left;">
            ...
            <c:set var="similar" value="<%=de.imi.odmtoolbox.model.ODMCompareType.SIMILAR%>"/>
            <c:forEach items="${comparedItems[similar]}" var="comparedItem"> 
                <tr>
                    <td>
                    ...
                    <c:forEach items="${comparedItem.getSimilarFormItems()}" var="form">
...
<c:if test="${studyForms == null || studyForms.isEmpty() == true || formWidth == null || formWidth.isEmpty() == true}">
    <p>The submitted ODM-Forms do not contain any similar items.</p>
</c:if>  